...
### Phase 1：奴隸循環核心（Part E） — ✅ 完成（11/11）
...
| 1-E.2 | `hunger_critical` 三選一事件 | ✅ | `77c50b0` | 實際做成 4 選項（多加鐵意志專屬選項），食物 ≤14 自動觸發 |
...
| 項目 | 狀態 | 備註 |
|------|------|------|
| ChoiceModal UI 系統 | ✅ 完成 `77c50b0` | `choice_modal.js` 完整支援：條件過濾、加權 rolls、強制模式、flag 寫入 |
...
### 📏 Flavor 文字長度約定（2026-04-15 定案）

**關係圖卡片**採用 2 欄網格，每張卡片的 flavor 文字有顯示空間限制：

| 長度 | 顯示效果 | 建議 |
|------|---------|------|
| **< 25 字** | 一行剛好 | ⭐ 理想（一目了然） |
| **25~45 字** | 兩行恰好 | ⭐ 理想（有呼吸感） |
| **46~55 字** | 剛好兩行滿 | 🟡 可接受但接近上限 |
| **> 55 字** | 被 `line-clamp: 2` 截掉，尾端 `...` | ❌ 會被吃掉，要避免 |

**寫作建議範圍**：**每段 flavor 25~45 個中文字**。

- 太短（< 20 字）沒有味道、讀來像提示工具
- 太長（> 55 字）會被 CSS 自動截斷，玩家永遠看不到完整句子
- 如果一個意象太長，拆成兩段 reveal 放不同好感門檻

**範例（卡西烏斯）**
- ✅ 「他很少說話。每天揮劍的時間永遠比別人多一個小時。」（25 字）
- ✅ 「他枕頭下似乎壓著一塊老舊的符牌。邊角被磨得發亮，像被人反覆摩挲了幾千次。」（37 字）
- ❌「他很少說話。每天揮劍的時間比別人多一小時，每次都揮到手臂抬不起來才肯停下，眼神始終凝視某個遠方不存在的點...」（57 字，會被截斷）
...
## 🆕 D.16 訓練所差異化系統（Phase 2 S2 設計）

> **狀態**：2026-04-15 設計討論定案，Phase 2 S2 正式啟動時實作。
> 這是遊戲從「一個訓練所的日常」擴充到「世界觀網絡」的關鍵。
...
- 破訓練所 = 難度高 + 加成少
- 豪華訓練所 = 難度低 + 加成多
- 玩家一看就知道「新手選破、老手選豪華」→ 選擇降維為難度條

本作設計：**每個訓練所都有別處拿不到的東西，破訓練所必須藏顆珍珠**。

### 四個訓練所的草案

| 訓練所 | 核心主題 | 屬性上限偏好 | 食物動態 | 專屬 NPC |
|--------|---------|-------------|---------|---------|
| **鐵獄** | 放任、無紀律、藏著傳奇 | WIL+3 / LUK+2 / STR-2 | 貧乏（基礎 12） | 退役魔神訓練官 |
| **鷹之營** | 斯巴達紀律 | STR+2 / CON+2 / AGI-2 | 充足（基礎 18）| 百夫長教官 |
| **標準訓練所** | 均衡生存（當前的） | 無修正 | 中等（基礎 18 調整後） | 監督官 Valerius |
| **安佩拉宅邸** | 貴族娛樂、政治 | WIL+2 / LUK+2 / CON-2 | 豐盛（基礎 25）| 占卜師 + 主人女兒 + 詩人 |

### 資料結構範例

```js
// facilities.js（Phase 2 S2 建立）
ironCage: {
  id: 'ironCage', name: '鐵獄訓練所',
  attrCapMod:      { STR:-2, DEX:-1, WIL:+3, LUK:+2 },      // 屬性上限修正
  trainingExpMult: { STR: 0.8, WIL: 1.3, LUK: 1.2 },        // 訓練效率修正
  mealGains: {                                              // 餐點數字
  ...
  sleepFoodCost:   -10,
  npcs: { cook: 'melaKook', overseer: null, ... },          // 此訓練所的核心 NPC
  uniqueNpcs:      ['retired_legend_grax'],                 // 只有這裡才有的 NPC
  masterPreference: null,                                    // 主人屬性偏好（影響好感）
  startingFlags:   ['facility_low_supervision'],
...
**Phase 2 S2 第一版**：鎖死在標準訓練所。玩家看到的選擇畫面：
...
**解鎖階段**（靠謠言推進）：
1. 完全未知 → `???` + 全黑剪影
2. 聽過名字（聽到 1 次謠言）→ 顯示名字 + 剪影還是黑
3. 聽過描述（聽到 3 次）→ 剪影顯示輪廓 + 一行描述
4. 親眼見過（被派遣橋接事件過去一次）→ 完整圖像 + 資料
5. **第一次通關後**永久解鎖可選
...
function spendExpOnAttr(attr) {
  ...
  if (cur >= getAttrCap(attr)) return false;  // 達上限不能升
  // ...
}
```
...
作為其他訓練所的**參考範本**，標準訓練所需要補：

1. **訓練官 NPC**：Valerius（瓦萊瑞斯）—— 穩健派退役老兵，相信「能活下來的人才是最強的」
2. **主人偏好**：masterArtus 偏好 **CON**（耐力）—— 呼應「生存優先」主題
3. **微小加成**：所有屬性訓練 EXP ×1.05（均衡就是特色）
4. **獨特事件**：新人教程儀式（帶新奴隸一天）

### 📋 D.16 實作 Check List

**階段 1：資料層**
- [ ] 建立 `facilities.js` 含 4 個訓練所草案
- [ ] `Stats.getAttrCap(attr)` + `spendExpOnAttr` 上限檢查
- [ ] `player.facility` 使用字串 ID（目前是 `null`）
- [ ] `Config.DAILY.MEAL_FOOD_GAIN` 等數字按訓練所 override

**階段 2：開局 UI**
- [ ] 開局流程：姓名 → origin → **facility**（現階段鎖死 stdTraining）→ 開場敘述
- [ ] 解鎖進度系統（`player.facilityKnowledge: { ironCage: 2, ... }`）
- [ ] 4 格卡片 UI + 暗影/剪影/文字三階段

**階段 3：擴充其他訓練所**
- [ ] 先做 **鐵獄**（驗證「窮中有寶」設計）
- [ ] 訓練官 NPC 資料 + storyReveals
- [ ] 專屬事件 + 橋接事件

**階段 4：標準訓練所升級為範本**
- [ ] 新增監督官 NPC：Valerius
- [ ] masterArtus 偏好 CON 設定
- [ ] 新人教程獨特事件

---

## 🆕 D.17 謠言系統（Phase 2 中期）

> **狀態**：2026-04-15 設計討論定案，與 D.16 訓練所系統互相依賴。
> 是遊戲從「訓練循環」擴充到「世界觀網絡」的**第二把鑰匙**。

### 核心概念

謠言 = **玩家可以「聽到」但不一定見到**的世界資訊。
...
**謠言系統能給的**：
1. **世界活起來** — 就算沒離開訓練所也知道外面在發生事情
2. **伏筆機制** — 讓未來事件有預告、有期待
3. **內容解鎖鑰匙** — 「聽過 X 次才能解鎖 Y」
4. **NPC 社交深度** — 他們對世界有意見、會八卦
5. **任務入口倉庫** — 每則謠言都是潛在任務種子

### 資料結構

```js
// rumors.js（Phase 2 S2 之後建立）
const RUMORS = {
  ironCage_legend: {
    id:       'ironCage_legend',
    text:     '聽說鐵獄藏了個老傢伙，徒手殺過十幾個人。',
    category: 'facility',          // facility / person / event / mystery
    sources:  ['market', 'guard', 'teammate'],   // 哪些通道會聽到
    minDay:   10,
    unlocks: [                      // 聽到 N 次解鎖什麼
  ...
  mela_nephew_missing: {
    id: 'mela_nephew_missing',
    text: '梅拉的老鄉村子被燒光了。有人說她的姪子被俘虜當奴隸賣了。',
    category: 'person',
    sources:  ['mela_chat'],
    minDay:   20,
    requireAffection: { melaKook: 50 },
    unlocks: [
      { heardCount: 1, flag: 'knows_mela_nephew_lost' },
      { heardCount: 1, questEnable: 'mela_find_nephew' },
    ],
  },
};

// 玩家端
Stats.player.heardRumors = {
  ironCage_legend: 2,
  mela_nephew_missing: 1,
};
```
...
| 通道 | 時機 | 頻率 |
|------|------|------|
| **市集跑腿** | errand 事件中附帶 | 每 7 天一次 |
| **訓練時隊友閒聊** | 訓練動作完成後 | 5% per action |
| **護衛抱怨** | 餐點事件附加 | 10% per meal |
| **梅拉主動說** | 好感 ≥ 50 時的對話 | 條件觸發 |
| **夢境** | D.13 睡前「獨自沉思」選項 | 20% |
| **新 NPC 加入** | 新奴隸入所帶來外部消息 | 觸發性 |

### 完整範例鏈：梅拉的姪子

這個範例展示謠言系統的終極價值：

1. **Day 20+**：聽到 flavor rumor「梅拉老鄉村子被燒光了」（市集跑腿）
2. **梅拉好感 ≥ 50**：開啟個人 rumor「她的姪子被賣為奴」（梅拉對話）
3. **觸發 action rumor**：梅拉悄悄拜託你「如果你有機會...」
4. **後續事件**：主人派你去鐵獄送信 → 發現一個新奴隸 → 名字與梅拉姪子相符
5. **完成任務**：帶回消息或人 → 梅拉好感 +30 → 姪子加入訓練所變隊友

**一條鏈串起：謠言 → 好感 → 任務 → 橋接事件 → 新 NPC**。這就是遊戲的深度。

### 看耳聞的 UI

- 關係圖 tab 底部加「耳聞」子區塊或獨立小 tab
- 列出已聽過的謠言（最多 10~15 條）
- 每條旁邊標「聽過 N 次」
- 分類篩選：地點 / 人物 / 事件 / 未解之謎
...
**階段 1：基礎資料層**
- [ ] 建立 `rumors.js` 含 10~15 條範本謠言
- [ ] `Stats.player.heardRumors: {}` 新欄位 + 新遊戲重置
- [ ] `rumors.hear(id)` helper（+1 計數、觸發 unlocks、寫 flag）

**階段 2：觸發通道**
- [ ] errand_market 結尾 50% 觸發隨機謠言
- [ ] 訓練動作 5% 觸發隊友閒聊謠言
- [ ] 餐點事件 10% 附帶護衛抱怨謠言

**階段 3：UI 顯示**
- [ ] 關係圖 tab 加「耳聞」子區
- [ ] 謠言卡片（category icon + text + 聽過次數）
- [ ] 過濾 / 排序

**階段 4：整合 D.16**
- [ ] 訓練所解鎖進度用 heardRumors 計算
- [ ] 開局選擇畫面顯示解鎖狀態
...
#### ✅ Phase 1-E.2：ChoiceModal + hunger_critical（commit `77c50b0`，2026-04-15）
- [x] `choice_modal.js` 通用玩家選擇 modal
- [x] 條件過濾：requireTrait / requireAnyTrait / requireAnyAilment / requireOrigin / requireMinAttr / requireFlag / requireAffection
- [x] 加權隨機結果（rolls）、強制模式（forced）、flagSet 寫入
- [x] `Events.CHOICE_EVENTS.hunger_critical` **4 選項**（忍著 / 乞討 / 偷竊 / 🆕 鐵意志專屬「用意志壓下飢餓」）
- [x] 乞討與偷竊都用 rolls 做加權隨機結果（3 分支 / 2 分支）
- [x] `main.js _checkHungerCritical()` 食物 ≤14 且當日未觸發過自動彈出
- [x] `hunger_critical_day_N` flag 防重複觸發
- [x] `Game.testChoice(id)` debug 指令
...
#### ✅ Phase 1-E.2 / E.11：ChoiceModal UI 系統（已實作）

**狀態：完成（commit `77c50b0`，2026-04-15）**
> 下方保留當初 2026-04-13 的完整設計規格供參考。實作時比原設計多做了：
>   - 第 4 個選項（鐵意志專屬）
>   - 條件過濾擴充到 requireOrigin / requireOriginNot / requireAffection
>   - 乞討選項做成 3 分支 rolls（得到麵包 40% / 被啐 35% / 被羞辱 25%）
>   - 偷竊選項的失敗分支會扣梅拉與主人好感
>   - flagSet 支援選項級與 rolls 級（後者是每個 roll 結果都能設獨立 flag）
