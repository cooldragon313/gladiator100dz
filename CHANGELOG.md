...
#### 修改
- 傷勢（wounds）顯示方式從純文字列表改為 `.trait-tag` 標籤，與特性/疤痕/病痛同一區塊呈現
- 右欄移除獨立「傷勢」小節，所有狀態標籤統一顯示在「人物特質」
- 新增四種傷勢標籤顏色：
  - `trait-wound-light` — 暖琥珀（輕傷）
  - `trait-wound-medium` — 深橙（中傷）
  - `trait-wound-severe` — 血橙（重傷）
  - `trait-wound-special` — 深紅脈動（腦震盪/憂鬱症等特殊傷）
- 特殊傷加入 `title` tooltip 顯示效果說明
- 傷勢格式：`🩹 手臂・輕傷`（一般）、`⚠ 憂鬱症`（特殊）
- 新遊戲預設帶入「寬厚」特性與「失眠症」病痛
