...
const LordEvents = (() => {
  ...
  function _playBanquetFarmboy() {
    const lines = [
      ...
      { text: '（皮膚白、沒風吹日曬的痕跡。）' },
      { text: '（鬢角修過、下巴乾淨。）' },
      { text: '（——這不是農夫的臉。）', color: '#aa5050' },

      // ── 破綻 3：手 ────────────────────────
      { text: '（你低頭看瓦倫的手。手放在桌邊。）' },
      { text: '（指甲修過、指節沒曬痕、掌心⋯⋯沒繭。）' },
      { text: '（——這不是握過鋤頭的手。）', color: '#aa5050' },
      ...
      { text: '（——這是背過的台詞。）', color: '#aa5050' },
      { text: '（——他是被買來、教好了演這齣的。）', color: '#aa5050' },
      ...
      { text: '（他不是「敵國人」。）' },
      { text: '（他是自己人。）' },
      { text: '（他殺了你村、然後升官、現在還在這裡——演自己是英雄。）', color: '#8b3030' },
      { text: '（你的手指掐住酒壺。）' },
      { text: '（壺裡的酒晃了。）' },
      { text: '（沒人發現。你站好。）' },
      { text: '（——我要殺了他。）', color: '#8b3030' },
      { text: '（——我要讓所有人知道他做了什麼。）', color: '#8b3030' },
    ];
  ...
  function _tryDay75BackgroundChatter() {
    ...
    const lines = [
      ...
      { text: '（老兵 B 沒講話、把煙掐了。）' },
      { text: '（你低頭繼續掃地。耳朵卻在震。）' },
      { text: '（——所以那不是真的「敵國軍隊」。）', color: '#aa5050' },
      { text: '（——當年參戰的人、自己都活不下去。）', color: '#aa5050' },
    ];
    if (typeof DialogueModal !== 'undefined') {
      DialogueModal.play(lines, {
        onComplete: () => {
  ...
  function _tryDay85ArmorSlip() {
    ...
    const lines = [
      ...
      { text: '（皮帶扣是一樣的。）' },
      { text: '（銅釘排列⋯⋯一模一樣。）', color: '#aa5050' },
      { text: '（你以前以為是巧合。或記錯了。）' },
      { text: '（現在你知道。）' },
      { text: '（——那群人穿的、就是帝國軍隊的甲。）', color: '#8b3030' },
      { text: '（——只是把外面的記號改了。）', color: '#8b3030' },
    ];
    if (typeof DialogueModal !== 'undefined') {
      DialogueModal.play(lines, {
        onComplete: () => {
  ...
  function _tryDay92VillageFaces() {
    ...
    const lines = [
      ...
      { text: '（——他們每一張臉。）', color: '#aa5050' },
      { text: '（你不能讓他們白死。）' },
      { text: '（你不能讓提圖斯演英雄。）' },
      { text: '（八天。再撐八天。）', color: '#8b3030' },
    ];
    if (typeof DialogueModal !== 'undefined') {
      DialogueModal.play(lines, {
        onComplete: () => {
