...
> ⚠️ **這是遊戲的核心架構。所有新功能都要對照這裡確認屬於哪一幕。**

**第一幕 · 訓練所篇（Day 1-30）—— 敘事開場**
- 定位：互動小說 + 角色 build。類比：實況野球的高校篇 / NBA 2K MyCareer 故事模式
- 玩家是奴隸，沒自主權。訓練、NPC 互動、道德選擇
- 探索世界、解鎖成就、build 出自己想要的角色原型（屬性/武器/特性/關係）
- 核心情感：奧蘭主線、震撼教育（Day 5 有人會死）、生存壓力
- 結束時「畢業」— 帶著你 build 的人進入第二幕

**第二幕 · 競技場篇（Day 31-100）—— 戰鬥解放**
- 定位：用你打造的角色去征服世界。類比：實況野球帶自創球員打甲子園
- 名聲提升 → 自由度增加（自由切磋、挑戰賽、押運任務、裝備鍛造）
- 戰鬥頻率大幅提高（每 2-3 天一場）
- 裝備系統完整開放（鍛造/掉落/強化）
- 故事線繼續（奧蘭後續、主人的野心、跨訓練所交流）
- Day 100 萬骸祭 = 最終 Boss

**後日談 · 自由模式（Day 100+ 存活後解鎖）**
- 定位：Gladiatus / 暗黑破壞神式的 endgame 循環
- 自由挑戰各競技場、裝備農場、隨機掉落 + 稀有度
- 解鎖其他訓練所（重走第一幕但不同環境、不同 NPC）
- 收集成就 / 解鎖角色 / Meta-progression

**為什麼這樣分**：
- 第一幕用故事抓住玩家的**心**（情感投入）
- 第二幕用戰鬥和裝備抓住玩家的**手**（成長快感）
- 後日談給核心玩家**無限循環**
- 第一幕 build 的角色會影響第二幕（屬性→戰鬥風格、武器→起點、道德→NPC 反應、關係→搭檔系統）

### 第一幕 Day 1-30 世界節奏表

> **世界事件**是不管玩家做什麼都會發生的。**角色事件**由好感/屬性/flag 觸發。
> 兩者疊加 = 每天都有機會發生「新的事」。

| Day | 世界事件 | 氛圍 | 角色事件（條件觸發）|
|---|---|---|---|
| 1 | 新奴隸入場（你 + 奧蘭 + 索爾等一批人）| 恐懼、陌生 | 🔒 奧蘭 Day 1 初遇（強制）|
| 2-4 | 訓練開始。監督官觀察。 | 適應 | 武器獎勵（訓練 ≥6 次觸發）/ Day 4 保底 |
| 5 | **基礎考驗 — 三人進，兩人出** | 震撼 | 索爾事件（不可迴避的犧牲選擇）|
| 7-8 | 考驗後的第一個平靜期 | 哀悼 / 麻木 | 奧蘭藥房懸念（8% 隨機觸發）|
| 10 | **新人淘汰日**（表現最差的被帶走）| 緊張 | 你的 fame 決定你看到什麼 |
| 12-14 | 第一次被安排切磋 | 成長感 | 奧蘭散點事件、storyReveals |
| 15 | **監督官公開訓話**（宣布秋季大賽日程）| 目標感出現 | 訓練里程碑晨思 |
| 18-20 | **第二批奴隸到來**（新面孔加入） | 新鮮感 | 新背景角鬥士 |
| 22-24 | 又一輪切磋 / 或被安排表演 | 日常中有波動 | NPC 好感觸發事件 |
| 25 | **秋祭**（訓練所放半天假，梅拉做特別料理）| 溫暖 | 社交事件集中觸發 |
| 28-29 | 氣氛轉變：主人開始評估「誰值得留」 | 壓力 | 主人/監督官好感影響走向 |
| 30 | **畢業線** — 奧蘭房間升級 / 第二幕準備 | 過渡 | master_aff + fame 觸發分房 |

### 為什麼特別

跟同類型遊戲（實況野球、Princess Maker 等）的差別：

1. **奴隸沒有自主權**（Phase 1 哲學・限第一幕）——Day 1-30 玩家只能選訓練類型，不能主動找 NPC 說話、不能自由移動。Day 31+ 隨名聲增加逐步解鎖自由度
...
5. **三幕結構**——前期用故事抓住玩家的心，中後期用戰鬥和裝備抓住玩家的手

### 目前進度（2026-04-16）

- **Phase 0 基礎設施** ✅ 全做完（15/15）
- **Phase 1 奴隸循環核心** ✅ 全做完（11/11）
- **人物面板** ✅ 六角形屬性圖 + 關係圖 tab + 裝備動態 picker + 道德光譜
- **Stage 轉場系統** ✅ 就寢動畫、開場敘述、事件小過場、晨起過場（雞鳴 + 晨思）
- **對話系統** ✅ DialogueModal L2 手動制 + ChoiceModal 條件篩選
- **晨思系統** ✅ MorningThoughts 30+ 條（懸念/身體/社交/預感/反思/預設）
- **D.18 訓練協力** ✅ 屬性偏好 + 背景角鬥士 + favorWeight + ×15 cap
- **D.19 道德累積** ✅ 10 earned traits + 滑動窗口 + NPC 愛憎倍率
- **D.20 奧蘭主線** ✅ 永駐兄弟 + 四幕脊椎 + 10 storyReveals + 偷藥/訣別/房間升級
- **D.22 醫生老默** ✅ 治療系統 + DialogueModal 18 句 + 差異化治療
- **音效系統** ✅ Web Audio 合成 8 種內建音效
- **武器系統** 🟡 Day 3 選擇事件做好，裝備切換可用，鍛造/掉落待做
- **D.12 故事揭露** ✅ 卡西烏斯 + 奧蘭範本，其他 NPC 待補
- **D.14 / D.16 / D.17** ⚠️ 設計完成，實作待第二幕
...
### 1.1 檔案結構（2026-04-20 Phase 2 重組後）
...
│   └── character_roll.js, ending.js, main.js  ← top-level
│
├── docs/               # 所有設計文件
│   ├── INDEX.md        # 🆕 文件總目錄
│   ├── CANON.md        # 故事事實單一事實源
...
│   ├── DIALOGUE-MAP.md # 對白位置索引
│   ├── systems/        # 系統設計規格
│   ├── characters/     # NPC 完整檔案
│   ├── quests/         # 任務設計書
│   ├── philosophy/     # 設計哲學
│   ├── discussions/    # 🆕 臨時討論紀錄
│   └── archive/        # 🆕 DESIGN-legacy.md / HISTORY.md / Phase1-D-I 老檔
│
├── asset/              # 圖片 / 音樂
├── misc/               # 參考圖片 + 指令寫法.html
├── tests/              # ending_test.html / testbattle.html
├── memory/             # Claude auto-memory
└── DESIGN.md           # 本檔（設計單一事實源）
```

詳細載入順序見 [CLAUDE.md § 模組相依](../CLAUDE.md#模組相依)。
