...
### 2026-05-13 結束時狀態（Sol Arc Phase 1 擴展：賠帳 + 高利貸 + 強制勞務 + 債務纏身）

**✅ 今天主要成果**

#### 1. 救索爾 Phase 1 兩個 bug 修
- **錢包還有 24G bug**：救索爾時、主人沒收身上現有錢抵債（min(wallet, 200)）
- **奧蘭跌倒 200 下蓋掉沙洗 bug**：加 `day_story_claimed` flag 機制
  - `DayCycle.fireDayStart` priority ≥ 30 hook 若已 claim 跳過
  - 沙洗 `_triggerThreePersonTrial` 開頭 set
  - `_scanStoryEvents` check 旗、不掃 storyReveal
  - 結果：腳本進行中、所有隨機事件都不再冒出

#### 2. 老默賒帳系統（私財凍結期）
- 玩家沒錢 + `master_skim_active` → 老默賒帳
- 主人加倍敲詐（cost × 2 進 debt）
- 對白：「⋯⋯沒錢？沒關係。主人讓我看著你、不是讓你死。」

#### 3. 「債務纏身」特性 2 段升級
- `debt_ridden` (≥ 500)：訓練 +10% / 戰鬥首回合 +10% / 跑腿 +20% / mood 下限 -10 / 心情恢復 -20%
- `debt_crushed` (≥ 1000、絕望版)：上述加倍
- 主人 + 赫克特債務**合計**算 tier、自動套用 / 移除

#### 4. 賭博變動押注 + 赫克特高利貸
- 賭場可選押注 5/10/20/30 銅幣 / 場（之前固定 5）
- shells_game 難度提升（speedMult 0.9 → 0.85、最快場 0.72 → 0.60）
- **沒錢時**才出現「跟赫克特借」選項（user 反饋）
- 借錢額度 = 玩家對赫克特好感（aff 30 → max 借 30）
- 借款 ×2 立刻變債務（**借 30 → 欠 60**）
- 每 7 天利息 ×1.3（60 → 78 → 101 → 132...）

#### 5. 赫克特週收事件
- 每 7 天觸發、還不起 → 挨打 / 反抗 選擇
- **挨打**：HP 砍半 + mood -25 + shake-and-flash
- **反抗**：Battle.startFromConfig vs 赫克特（STR18/DEX25/CON18/HP110）
  - 贏：「不錯、不怕你還不完了」+ 利息照算
  - 輸：HP=5 + mood -30 + 「就這身手還反抗、欠扁！」

#### 6. 還債順序鏈
- modMoney 攔截正向金額、依序扣抵：
  1. 主人債（`master_skim_active`）
  2. 主人清完才開始扣赫克特債
- 兩段都觸發 `_refreshDebtTrait()`

#### 7. 強制勞務系統
- 凍結期 + debt ≥ 100 + 每天 8% 機率隨機派遣
- 對白：侍從來、搬石頭一天
- 效果：stamina -40 / debt -15 / STR+CON EXP +6

#### 8. UI 債務名單
- 角色頁右欄、債務 section
- 主人 / 赫克特債務分行顯示、額度為 0 時隱藏

**🔮 下次開工優先序**
1. **🐛 測試 Phase 1 全套**（救索爾 + 賒帳 + 賭博 + 高利貸 + 強制勞務 + 債務纏身）
   - `Game.testJump('day5')` + `Game.godMode({attr:30})`
   - 戰勝索爾饒他 → 確認債務名單顯示、24G 被沒收
   - 故意受傷 → 找老默 → 看賒帳對白
   - 等晚上 + 沒錢 → 看「跟赫克特借」選項
   - 借錢賭、輸 → 看赫克特債 +60
   - 隨便 testJump 一週 → 看赫克特利息 ×1.3 + 週收事件
   - 反抗赫克特 → 戰鬥
2. **Phase 2 索爾養傷醒 + 偷懶遮掩 + 守夜情報 + CON 陪練**（Sol arc 下一階段）
3. **NPC 對白變調**（debt > 500 時、5 個 NPC 講話風格變）— 之前討論未做

**⚠️ Phase 1 已知限制**
- 葛拉自願苦力沒做（要加進行動列表）
- 賭場 NPC 邀請事件還是 12% 機率 / 5 天 cooldown、user 可能想再調

**⚠️ 設計漂移待修（沿用昨天）**
- 採臉 vs 放過 對白不對齊（招敵變友 seedLines「我欠你一次」）
- 等實作仇恨度四選一時、要把現有「採臉」對白同步改成新 spec
